...
function showEndScreen() {
  ...
  document.getElementById("end-score").textContent =
    `${sessionSolved} / ${ROUNDS_PER_SESSION}`;
  ...
  for (let f = 0; f < frameCount; f++) {
    ...
    for (let y = 0; y < gridSize; y++) {
      for (let x = 0; x < gridSize; x++) {
        if (x < gridSize - 1 && Math.random() < 0.15)
          edges.push({ x1: x, y1: y, x2: x + 1, y2: y });
        if (y < gridSize - 1 && Math.random() < 0.15)
...
function startLevel() {
  ...
  const filledFaces =
    cornerTriples[Math.floor(Math.random() * cornerTriples.length)];
  ...
  const targetSequence =
    targetView === "inside"
      ? generatedSequences[targetFaceSequenceIndex].map((frame) =>
          frame.map((s) => ({ x1: -s.x1, y1: s.y1, x2: -s.x2, y2: s.y2 })),
        )
...
function resolveLevel(selectedIndex) {
  ...
  targetQuaternion.setFromEuler(
    new THREE.Euler(targetXRot, targetYRot, 0, "YXZ"),
  );
  ...
  if (currentFrameIndex !== lastDrawnFrame && currentOptions.length > 0) {
    ...
    cards.forEach((card, index) => {
      if (currentOptions[index]) {
        ...
        draw2DSegments(
          canvas,
          currentOptions[index].sequence[currentFrameIndex],
        );
      }
    });
  }
