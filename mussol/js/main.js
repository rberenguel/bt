...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const parseMarkdownQuestions = (text) => {
    ...
    lines.forEach((line) => {
      ...
      if (line.startsWith("#")) {
        ...
        const stemText = line.replace(/^#\s*(?:\d+\.\s*)?/, "").trim();
        const parts = stemText.split(",").map((p) => p.trim());
        if (parts.length >= 2) {
          currentQuestion = {
        ...
        if (parts.length >= 2) {
          if (isCorrect) {
            currentQuestion.answer = currentQuestion.choice.length;
          }
          currentQuestion.choice.push([parts[0], parts[1]]);
        }
      }
    });
  ...
  const loadAllData = async () => {
    try {
      const response = await fetch("./data/questions.md");
      if (!response.ok) {
      ...
      loaderEl.textContent = "Could not load questions.";
      return;
    }
