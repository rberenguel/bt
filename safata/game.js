...
const P_REWORK                = 0.60; // Probability a completed tile re-lights next turn
    ...
    tiles.forEach(tile => {
        ...
        if (!tile.active) {
            ...
            btn.innerHTML =
                `<i class="ph-light ${tile.icon}"></i>` +
                `<span class="cc-tile-label">${tile.label}</span>`;
            btn.addEventListener("click", e => { e.preventDefault(); handleCCTap(tile.id); }, { passive: false });
        } else {
...
function endSession() {
    ...
    const redAccuracy    = met.redTotalTaps > 0
        ? Math.round((1 - met.redWrongTaps / met.redTotalTaps) * 100)
        : 100;
    const yellowAccuracy = met.yellowTotalTaps > 0
        ? Math.round((1 - met.yellowWrongTaps / met.yellowTotalTaps) * 100)
        : 100;
    const tilesReachedRed  = activeTiles.filter(t => t.visitCount >= RULE_VISIBLE_VISITS).length;
    const avgVisitsPerTile = activeTiles.length > 0
    ...
    DOM.oStats.innerHTML = [
        { label: "Red accuracy",       value: `${redAccuracy}%`,        note: `${met.redWrongTaps} wrong of ${met.redTotalTaps} taps` },
        { label: "Yellow accuracy",    value: `${yellowAccuracy}%`,     note: "rule visible" },
        { label: "Contexts memorised", value: `${tilesReachedRed}`,     note: "tiles reached recall state" },
