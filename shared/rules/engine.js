...
export const ICONS = ["ph-alien", "ph-bug", "ph-ghost", "ph-robot", "ph-rocket", "ph-skull"];
...
export const FILTER_RULES = [
    ...
    ...ICONS.map(ico => ({ id: `ICO_${ico}`,  text: `Tap ${ico.replace("ph-","")}`, dim: "icon",   isValid: (t) => t.icon === ico })),
...
export function generateBoard(rules) {
    ...
    for (const rule of rules) {
        ...
        if (!board.some(t => rule.isValid(t, board))) {
            forceTileToMatchRule(board[Math.floor(Math.random() * board.length)], rule);
        }
    }
...
export function pickRules(count, existingRules = []) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (available.length === 0) {
            available = FILTER_RULES.filter(r => !allUsed.some(sr => sr.id === r.id));
        }
