...
export function injectFakeHistory(days = 30) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    for (let j = 0; j < dailySessions; j++) {
      const accuracy = Math.round(Math.min(100, Math.max(30, baseAccuracy + (Math.random() * 16 - 8))));
      records.push({
        timestamp: ts + j * 3_600_000,
        dateStr: new Date(ts).toISOString().split("T")[0],
        metrics: {
          accuracy,
          score:      Math.round(Math.min(30, Math.max(10, baseScore      + (Math.random() * 6 - 3)))),
          maxStreak:  Math.round(Math.min(20, Math.max(1,  baseMaxStreak  + (Math.random() * 4 - 2)))),
          finalLevel: Math.round(Math.min(7,  Math.max(1,  baseFinalLevel + (Math.random() * 2 - 1)))),
        },
      });
    }
    ts += 24 * 60 * 60 * 1000;
